...
#Data$Gender<-revalue(Data$Gender, c("a" = "M", "b" = "F"))
#Data
...
print(crx$Age)
#round(Data$Age, digits = 0)
#Age<-c(Data$Age)
#Age
crx$MonthlyExpenses <- as.integer(crx$MonthlyExpenses)
print(crx$MonthlyExpenses)
#MonthlyExpenses<-c(Data$MonthlyExpenses)
#MonthlyExpenses
...
crx$BankingInstitution
crx$YearsEmployed <- as.numeric(crx$YearsEmployed)
crx$YearsEmployed
#round(Data$YearsEmployed, digits = 0)
...
crx$Approved
str(crx)
#Question 5:Records with missing values. There are also a few missing values. 
...
#Table 7 The dataset contains variables with mixed types. Use R function daisy() from package cluster to compute a 
#Gower dissimilarity (distance) matrix between the data records, and refer to the result as “Dist”.  
#Enter the R code you used, including any libraries needed.
library(cluster)
...
#Table 9 Using the new distance matrix, what is the Gower similarity measure between the 10th and the 60th observation (row)? 
#Answer using R command(s).
Dist_matrix[c(10,60),]
...
#Table 12 Enter your R code used to calculate the Pearson and then the Spearman correlation matrices 
...
#Table 13 Use the ggplot2 library to produce box plots for AccountBalance, MonthlyExpenses, CreditScore and Age 
...
ggplot(data = crx_omit) + aes(x=Approved, y=Age) + 
  aes(color = Approved) + geom_boxplot(outlier.size = 2,
                                       outlier.color = "red", outlier.shape = 3) + 
  geom_jitter(width = 0.1, alpha = 0.05, color = "blue")
#Table 15 Use the ggplot2 library to produce bar plots for Employed, MaritalStatus, BankingInstitution, and NoPriorDefault, all segmented by approval (variable “Approved”). 
#Insert the R codes and resulting images into the table below.
...
#Boxplot MonthlyExpense versus Approved
